...
export default function Home() {
  ...
  const filePresignMutation = trpc.files.presign.useMutation();
  const fileCompleteMutation = trpc.files.complete.useMutation();
  ...
  const unreadChatQuery = trpc.social.unreadCount.useQuery(undefined, { enabled: isAuthenticated, refetchInterval: 5000, retry: false });
  ...
  const unreadMessageCount = unreadChatQuery.data ?? 0;
  ...
  const uploadFileDirect = async (file: File) => {
    if (file.size > 35 * 1024 * 1024) throw new Error("Files must be 35 MB or smaller.");
    const mimeType = file.type || "application/octet-stream";
    const prepared = await filePresignMutation.mutateAsync({ name: file.name, mimeType, sizeBytes: file.size });
    const response = await fetch(prepared.uploadUrl, { method: "PUT", headers: { "Content-Type": mimeType }, body: file });
    if (!response.ok) throw new Error(`Storage upload failed (${response.status})`);
    return fileCompleteMutation.mutateAsync({ name: file.name, mimeType, storageKey: prepared.key, sizeBytes: file.size });
  };
  ...
  const submitChatMessage = async () => {
    ...
    const uploaded = chatFile ? await uploadFileDirect(chatFile) : undefined;
    ...
    <div className="app-shell">
      <aside className={`side-rail ${mobileNav ? "mobile-open" : ""}`}>
        ...
        <nav className="secondary-nav">
          <button className="nav-item" onClick={() => requireAuth("find and add friends", () => setLocation("/friends"))}><UserPlus size={18} /><span>Add friends</span>{pendingFriendRequestCount > 0 && <span className="count-badge">{pendingFriendRequestCount > 99 ? "99+" : pendingFriendRequestCount}</span>}</button>
          <button className="nav-item" onClick={() => requireAuth("view saved videos")}><Bookmark size={18} /><span>Saved</span></button>
          <button className="nav-item" onClick={() => requireAuth("view your files", () => setLocation("/files"))}><FolderOpen size={18} /><span>My files</span></button>
      ...
      <main className="main-stage">
        <header className="topbar">
          ...
          <div className="top-actions">
            <button className="icon-button notification-button" aria-label={unreadNotificationCount ? `${unreadNotificationCount} unread notifications` : "Notifications"} onClick={() => requireAuth("view notifications", () => setLocation("/notifications"))}><Bell size={18} />{unreadNotificationCount > 0 && <i />}</button>
        ...
        <div className="content-wrap">
          ...
          {section === "discover" && <section className="content-section discover-section"><div className="discover-intro"><div><span className="eyebrow"><Compass size={13} /> Network discovery</span><h2>Discover real members.</h2><p>Search for people who have actually joined VirgoX and connect with them.</p></div><button className="hero-button" onClick={() => requireAuth("find members", () => setLocation("/friends"))}><Users size={15} /> Find members</button></div><div className="friend-empty"><Users size={28} /><strong>No public recommendations yet</strong><p>VirgoX does not invent creators or trends. Use Add friends to search real accounts.</p><button className="hero-button" onClick={() => requireAuth("find members", () => setLocation("/friends"))}><Search size={15} /> Search members</button></div></section>}
