  ...
  if (!_db && process.env.DATABASE_URL) {
    try {
      ...
      _schemaReady = (async () => {
        ...
        await sqlClient`CREATE TABLE IF NOT EXISTS stored_files (id serial PRIMARY KEY, "ownerId" integer NOT NULL REFERENCES users(id) ON DELETE CASCADE, "groupId" integer, "originalName" varchar(255) NOT NULL, "storageKey" text NOT NULL, "mimeType" varchar(120) NOT NULL, "sizeBytes" integer NOT NULL, "scanStatus" varchar(64) NOT NULL DEFAULT 'pending', visibility varchar(64) NOT NULL DEFAULT 'private', "createdAt" timestamptz NOT NULL DEFAULT now())`;
        await sqlClient`CREATE TABLE IF NOT EXISTS messages (id serial PRIMARY KEY, "conversationId" integer NOT NULL REFERENCES conversations(id) ON DELETE CASCADE, "senderId" integer NOT NULL REFERENCES users(id) ON DELETE CASCADE, body text, "fileId" integer, "createdAt" timestamptz NOT NULL DEFAULT now(), "deliveredAt" timestamptz, "readAt" timestamptz)`;
        await sqlClient`ALTER TABLE messages ADD COLUMN IF NOT EXISTS "deliveredAt" timestamptz`;
        await sqlClient`ALTER TABLE messages ADD COLUMN IF NOT EXISTS "readAt" timestamptz`;
        await sqlClient`CREATE TABLE IF NOT EXISTS chat_presence ("userId" integer PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE, "lastSeenAt" timestamptz NOT NULL DEFAULT now(), "typingConversationId" integer, "typingUntil" timestamptz)`;
